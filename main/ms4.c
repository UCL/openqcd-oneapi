...
static char cnfg_file[2 * NAME_SIZE + 16], sfld_file[2 * NAME_SIZE + 16],
    nbase[NAME_SIZE];
...
static void read_dirs(void)
{
  if (my_rank == 0) {
    ...
    error_root((last < first) || (step < 1) || (((last - first) % step) != 0),
               1, "read_dirs [ms4.c]", "Improper configuration range");
  }
...
static void setup_files(void)
{
  ...
  error_root(name_size("%s/%s.ms4.log~", log_dir, nbase) >= NAME_SIZE, 1,
...
static void solve_dirac(spinor_dble *eta, spinor_dble *psi, int *status)
{
  ...
  if (sp.solver == CGNE) {
    ...
    error_root((status[0] < 0) || (status[1] < 0), 1, "solve_dirac [ms4.c]",
