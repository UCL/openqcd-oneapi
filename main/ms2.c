...
static void read_dirs(void)
{
  if (my_rank == 0) {
    ...
    error_root((last < first) || (step < 1) || (((last - first) % step) != 0),
               1, "read_dirs [ms2.c]", "Improper configuration range");
  }
...
static void read_infile(int argc, char *argv[])
{
  ...
  if (my_rank == 0) {
    ...
    error_root(endian == openqcd_utils__UNKNOWN_ENDIAN, 1, "read_infile [ms2.c]",
...
int main(int argc, char *argv[])
{
  ...
  if (my_rank == 0) {
    ...
    eps = ra / rb;
    eps = eps * eps;
    Ne = 0.5 * (double)(NPROC0 * L0 - 2) *
         (double)(NPROC1 * NPROC2 * NPROC3 * L1 * L2 * L3);
