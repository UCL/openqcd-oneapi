...
static void read_dirs(void)
{
  ...
  if (my_rank == 0) {
    ...
    error_root((last < first) || (step < 1) || (((last - first) % step) != 0),
               1, "read_dirs [ms1.c]", "Improper configuration range");
  ...
  error(nfct == NULL, 1, "read_dirs [ms1.c]", "Unable to allocate data array");
  file_head.nrw = nrw;
  file_head.nfct = nfct;
...
static void setup_files(void)
{
  if (noexp) {
    ...
    error_root(name_size("%s/%sn%d", cnfg_dir, nbase, last) >= NAME_SIZE, 1,
               "setup_files [ms1.c]", "cnfg_dir name is too long");
  }

  check_dir_root(log_dir);
  check_dir_root(dat_dir);
  error_root(name_size("%s/%s.ms1.log~", log_dir, nbase) >= NAME_SIZE, 1,
             "setup_files [ms1.c]", "log_dir name is too long");
  error_root(name_size("%s/%s.ms1.dat~", dat_dir, nbase) >= NAME_SIZE, 1,
...
static void check_old_log(int *fst, int *lst, int *stp)
{
  ...
  while (fgets(line, NAME_SIZE, fend) != NULL) {
    if (strstr(line, "process grid") != NULL) {
      if (sscanf(line, "%dx%dx%dx%d process grid, %dx%dx%dx%d", np, np + 1,
...
static void check_old_dat(int fst, int lst, int stp)
{
  ...
  error_root(ic == 0, 1, "check_old_dat [ms1.c]", "No data records found");
  error_root((ie & 0x1) != 0x0, 1, "check_old_dat [ms1.c]",
             "Configuration numbers are not equally spaced");
  error_root((fst != fc) || (lst != lc) || (stp != dc), 1,
             "check_old_dat [ms1.c]",
...
static void print_info(void)
{
  ...
  if (my_rank == 0) {
    ...
    error_root(flog == NULL, 1, "print_info [ms1.c]",
               "Unable to open log file");
    printf("\n");
...
static void solver_wsize(int isp, int nsd, int np, int *nws, int *nwsd,
                         int *nwv, int *nwvd)
{
  solver_parms_t sp;
  ...
  for (irw = 0; irw < nrw; irw++) {
    ...
    if (rwp.rwfact == RWRAT) {
      ...
      for (isrc = 0; isrc < nsrc; isrc++) {
        lnr[isrc] =
            rwrat(rwp.irp, nfct, rwp.np, rwp.isp, sqn + isrc, rwstat + nfct);
...
int main(int argc, char *argv[])
{
  ...
  for (nc = first; (iend == 0) && (nc <= last); nc += step) {
    ...
    if (my_rank == 0) {
      ...
      error_root(fdat == NULL, 1, "main [ms1.c]", "Unable to open dat file");
      write_data();
      fclose(fdat);
    }
