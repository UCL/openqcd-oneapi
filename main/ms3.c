...
static void write_file_head(void)
{
  ...
  error_root(iw != 4, 1, "write_file_head [ms3.c]", "Incorrect write count");
}
...
static void check_file_head(void)
{
  ...
  error_root(
      ...
      1, "check_file_head [ms3.c]", "Unexpected value of dn,nn,tmax or eps");
}
...
static void read_dirs(void)
{
  if (my_rank == 0) {
    ...
    error_root((last < first) || (step < 1) || (((last - first) % step) != 0),
               1, "read_dirs [ms3.c]", "Improper configuration range");
  }
...
static void setup_files(void)
{
  if (noexp)
    ...
    error_root(name_size("%s/%sn%d", cnfg_dir, nbase, last) >= NAME_SIZE, 1,
               "setup_files [ms3.c]", "cnfg_dir name is too long");

  check_dir_root(log_dir);
  check_dir_root(dat_dir);
  error_root(name_size("%s/%s.ms3.log~", log_dir, nbase) >= NAME_SIZE, 1,
...
static void read_wflow_parms(void)
{
  ...
  if (my_rank == 0) {
    ...
    error_root((dnms < 1) || (nstep < dnms) || ((nstep % dnms) != 0), 1,
               "read_wflow_parms [ms3.c]", "nstep must be a multiple of dnms");
  }
...
static void check_old_dat(int fst, int lst, int stp)
{
  ...
  error_root(ic == 0, 1, "check_old_dat [ms3.c]", "No data records found");
...
static void print_info(void)
{
  ...
  if (my_rank == 0) {
    ...
    error_root(flog == NULL, 1, "print_info [ms3.c]",
               "Unable to open log file");
    printf("\n");
