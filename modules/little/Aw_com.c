...
static void set_snd_req_bsd(void)
{
  ...
  for (ifc = 0; ifc < 8; ifc++) {
    ...
    MPI_Send_init((*brd).snd_buf[(ifc & 0x1) ^ 0x1][0], nbf, MPI_DOUBLE, saddr,
                  tag, MPI_COMM_WORLD, &snd_req_bsd[ifc]);
    MPI_Recv_init((*b2b).sde[(ifc & 0x1) ^ 0x1][0], nbf, MPI_DOUBLE, raddr, tag,
                  MPI_COMM_WORLD, &rcv_req_bsd[ifc]);
  }
}
...
static void alloc_bsd(void)
{
  ...
  error((bsd == NULL) || (iud == NULL) || (ud == NULL) || (psd == NULL) ||
            (sd == NULL),
        1, "alloc_bsd [Aw_com.c]", "Unable to allocate buffers");
...
static void set_snd_req_Aoe(void)
{
  ...
  for (ifc = 0; ifc < 8; ifc++) {
    ...
    MPI_Send_init(Aw.Aoe[8 * nbh + obbo[ifc] - nbbh], nbf, MPI_DOUBLE, saddr,
                  tag, MPI_COMM_WORLD, &snd_req_Aoe[ifc]);
    MPI_Recv_init(rcv_buf_Aoe[ifc][0], nbf, MPI_DOUBLE, raddr, tag,
                  MPI_COMM_WORLD, &rcv_req_Aoe[ifc]);

    tag = mpi_permanent_tag();

    MPI_Send_init(Aw.Aeo[8 * nbh + obbo[ifc] - nbbh], nbf, MPI_DOUBLE, saddr,
                  tag, MPI_COMM_WORLD, &snd_req_Aeo[ifc]);
    MPI_Recv_init(rcv_buf_Aeo[ifc][0], nbf, MPI_DOUBLE, raddr, tag,
                  MPI_COMM_WORLD, &rcv_req_Aeo[ifc]);
  }
}
...
static void alloc_Aoe(void)
{
  ...
  error((pzd == NULL) || (zd == NULL), 1, "alloc_Aoe [Aw_com.c]",
...
static void add_mat(int ifc, int vol, int *imb, complex_dble **v,
                    complex_dble **w)
{
  int nmat;
...
void cpAoe_ext_bnd(void)
{
  ...
  for (n = 0; n < nsnd; n++) {
    ...
    add_mat(io ^ 0x1, nbbe[io ^ 0x1], ipp + obbe[io ^ 0x1], rcv_buf_Aoe[io],
            Aw.Aoe);
    ...
    add_mat(io ^ 0x1, nbbe[io ^ 0x1], ipp + obbe[io ^ 0x1], rcv_buf_Aeo[io],
            Aw.Aeo);
...
static void set_snd_req_Aee(void)
{
  ...
  for (ifc = 0; ifc < 8; ifc++) {
    ...
    MPI_Send_init(snd_buf_Aee[ifc][0], nbf, MPI_DOUBLE, saddr, tag,
                  MPI_COMM_WORLD, &snd_req_Aee[ifc]);
    MPI_Recv_init(Aw.Aee[nbh + obbe[ifc ^ 0x1]], nbf, MPI_DOUBLE, raddr, tag,
                  MPI_COMM_WORLD, &rcv_req_Aee[ifc]);
  }
}
...
static void alloc_Aee(void)
{
  ...
  error((pzd == NULL) || (zd == NULL), 1, "alloc_Aee [Aw_com.c]",
        "Unable to allocate buffers");
