                          ...
                          int nlv, double tau)
{
  ...
  if (NPROC > 1) {
    ...
    error((iprms[0] != nact) || (iprms[1] != npf) || (iprms[2] != nmu) ||
              (iprms[3] != nlv) || (dprms[0] != tau),
          1, "set_hmc_parms [hmc_parms.c]", "Parameters are not global");
  ...
  error_root((hmc.nlv > 0) && (npf != hmc.npf), 1,
...
void write_hmc_parms(FILE *fdat)
{
  ...
  if (my_rank == 0) {
    ...
    error_root((istd == NULL) || (dstd == NULL), 1,
               "write_hmc_parms [hmc_parms.c]",
