...
static void check_block_size(int const *bs)
{
  ...
  error_root((bs[0] < 4) || (bs[1] < 4) || (bs[2] < 4) || (bs[3] < 4) ||
             ...
             1, "check_block_size [sap_parms.c]",
             "Block sizes are out of range");

  error_root((bs[0] % 2) || (bs[1] % 2) || (bs[2] % 2) || (bs[3] % 2), 1,
             "check_block_size [sap_parms.c]", "Block sizes must be even");
  ...
  error_root((n0 * n1 * n2 * n3) % 2, 1, "check_block_size [sap_parms.c]",
...
sap_parms_t set_sap_parms(int const *bs, int isolv, int nmr, int ncy)
{
  ...
  if (NPROC > 1) {
    ...
    error((iprms[0] != bs[0]) || (iprms[1] != bs[1]) || (iprms[2] != bs[2]) ||
              ...
              (iprms[6] != ncy),
          1, "set_sap_parms [sap_parms.c]", "Parameters are not global");
  }

  if (sap.ncy > 0) {
    error_root((bs[0] != sap.bs[0]) || (bs[1] != sap.bs[1]) ||
                   (bs[2] != sap.bs[2]) || (bs[3] != sap.bs[3]),
               1, "set_sap_parms [sap_parms.c]", "bs[4] may be set only once");
  } else {
    check_block_size(bs);
    sap.bs[0] = bs[0];
    sap.bs[1] = bs[1];
