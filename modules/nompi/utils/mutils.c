...
int digits(double x, double dx, char *fmt)
{
  error(dx < 0.0, 1, "digits [mutils.c]",
...
long find_section(FILE *stream, char *title)
{
  ...
  error(sofs == -1L, 1, "find_section [mutils.c]", "Section [%s] not found",
        title);
  ie = fseek(stream, sofs, SEEK_SET);
  error(ie != 0, 1, "find_section [mutils.c]", "Unable to go to section [%s]",
        title);
  get_line(stream);
...
static long find_tag(FILE *stream, char *tag)
{
  ...
  error(ie != 0, 1, "find_tag [mutils.c]", "Unable to go to line with tag %s",
        tag);
