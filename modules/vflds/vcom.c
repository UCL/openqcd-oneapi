...
static void alloc_vbufs(void)
{
  ...
  for (ifc = 0; ifc < 8; ifc++) {
    ...
    MPI_Send_init(snd_buf_int[ifc], 2 * Ns * nbbo[ifc], MPI_FLOAT, saddr, tag,
                  MPI_COMM_WORLD, &snd_req_int[ifc]);
    MPI_Recv_init(rcv_buf_int[ifc], 2 * Ns * nbbe[ifc ^ 0x1], MPI_FLOAT, raddr,
  ...
  for (ifc = 0; ifc < 8; ifc++) {
    ...
    MPI_Send_init(snd_buf_ext[ifc], 2 * Ns * nbbe[ifc], MPI_FLOAT, saddr, tag,
                  MPI_COMM_WORLD, &snd_req_ext[ifc]);
    MPI_Recv_init(rcv_buf_ext[ifc], 2 * Ns * nbbo[ifc ^ 0x1], MPI_FLOAT, raddr,
...
void cpv_int_bnd(complex *v)
{
  ...
  for (n = 0; n < nsnd; n++) {
    ...
    if (sflg[io ^ 0x1])
      assign_v2v(Ns * nbbe[io ^ 0x1], rcv_buf_int[io],
                 vb + Ns * obbe[io ^ 0x1]);
    else
