...
void chexp_drv1(su3_alg_dble const *X, ch_drv1_t *s)
{
  ...
  for (n = N - 4; n >= 0; n--) {
    __asm__ __volatile__("movapd %%xmm2, %%xmm6 \n\t"
                         ...
                         "addsd %1, %%xmm6"
                         :
                         : "m"(_sse_sgn1_dble), "m"(c[n])
                         : "xmm2", "xmm6", "xmm7", "xmm8", "xmm9");
...
void chexp_drv2(su3_alg_dble const *X, ch_drv2_t *s)
{
  ...
  (*s).ptt[1].re =
      -2.0 * (*s).pd[2].im + t * (*s).pdd[1].re - d * (*s).pdd[2].im;
  (*s).ptt[1].im =
      2.0 * (*s).pd[2].re + t * (*s).pdd[1].im + d * (*s).pdd[2].re;
