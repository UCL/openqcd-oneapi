...
static void alloc_sbufs(void)
{
  ...
  for (ifc = 0; ifc < 8; ifc++) {
    ...
    if (nbf[ifc] > 0) {
      ...
      MPI_Send_init(snd_buf[ifc], 12 * nbf[ifc], MPI_FLOAT, saddr, tag,
                    MPI_COMM_WORLD, &snd_req[ifc]);
      MPI_Recv_init(rcv_buf[ifc], 12 * nbf[ifc], MPI_FLOAT, raddr, tag,
                    MPI_COMM_WORLD, &rcv_req[ifc]);
    }
...
static void zip_weyl(int vol, spinor *pk, weyl *pl)
{
  ...
  for (; pl < pm; pl++) {
    __asm__ __volatile__("movaps %0, %%xmm0 \n\t"
                         ...
                         "movaps %4, %%xmm2"
                         :
                         : "m"((*pk).c1.c1), "m"((*pk).c1.c2), "m"((*pk).c1.c3),
                           "m"((*pk).c2.c1), "m"((*pk).c2.c2), "m"((*pk).c2.c3)
                         : "xmm0", "xmm1", "xmm2");

    __asm__ __volatile__("movaps %%xmm0, %0 \n\t"
                         "movaps %%xmm1, %2 \n\t"
                         "movaps %%xmm2, %4"
                         : "=m"((*pl).c1.c1), "=m"((*pl).c1.c2),
                           "=m"((*pl).c1.c3), "=m"((*pl).c2.c1),
                           "=m"((*pl).c2.c2), "=m"((*pl).c2.c3));
...
static void unzip_weyl(int vol, weyl *pk, spinor *pl)
{
  ...
  for (; pl < pm; pl++) {
    ...
    __asm__ __volatile__("movaps %%xmm5, %0 \n\t"
                         "movaps %%xmm6, %2 \n\t"
                         "movaps %%xmm7, %4"
                         : "=m"((*pl).c3.c1), "=m"((*pl).c3.c2),
                           "=m"((*pl).c3.c3), "=m"((*pl).c4.c1),
                           "=m"((*pl).c4.c2), "=m"((*pl).c4.c3));
