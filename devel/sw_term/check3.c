      ...
      for (x2 = 0; x2 < L2; x2++) {
        for (x3 = 0; x3 < L3; x3++) {
          ...
          if (((x[0] > 0) && ((x[0] < (N0 - 1)) || (bc != 0))) || (bc == 3)) {
            ...
            Fhat[ix][0] =
                0.25 * (ftp[0][0] + ftp[1][0] + ftp[2][0] + ftp[3][0]);
            Fhat[ix][1] =
                0.25 * (ftp[0][1] + ftp[1][1] + ftp[2][1] + ftp[3][1]);
            Fhat[ix][2] =
                0.25 * (ftp[0][2] + ftp[1][2] + ftp[2][2] + ftp[3][2]);
          } else {
            Fhat[ix][0] = 0.0;
            Fhat[ix][1] = 0.0;
            Fhat[ix][2] = 0.0;
...
static void muladd_pauli(double csw, int mu, int nu, spinor_dble *pk,
                         spinor_dble *pl)
{
  int ix;
  double r;
...
int main(int argc, char *argv[])
{
  ...
  for (n = 0; n < 4; n++) {
    ...
    if (my_rank == 0) {
      ...
      printf("t=%.2f,%.2f,%.2f, a=%.2f,%.2f,%.2f,%.2f, ", t[0], t[1], t[2],
             a[0], a[1], a[2], a[3]);
      printf("np=%d,%d,%d,%d\n", np[0], np[1], np[2], np[3]);
      printf("delta = %.2e\n\n", d);
    }
