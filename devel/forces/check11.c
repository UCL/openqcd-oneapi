...
static double dSdt(double mu0, double mu1, int ipf, int isw, int isp,
                   int *status)
{
  mdflds_t *mdfs;
...
int main(int argc, char *argv[])
{
  ...
  for (isw = 0; isw < 2; isw++) {
    for (isp = 0; isp < 3; isp++) {
      ...
      if (isp == 2) {
        dfl_modes(status);
        error_root(status[0] < 0, 1, "main [check11.c]", "dfl_modes failed");
      }
      ...
      error_root((status[0] < 0) || (status[1] < 0), 1, "main [check11.c]",
                 "action5 failed %d ", isp);
