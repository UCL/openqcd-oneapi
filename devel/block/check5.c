...
int main(int argc, char *argv[])
{
  ...
  for (n = 0; n < nb; n++) {
    for (iset = 0; iset <= (int)(PT_SETS); iset++) {
      ...
      error(check_sb(b, set, k, ps[0]), 1, "main [check5.c]",
            "assign_sblk2s() is incorrect");

      error(diff_s(vol, (*b).s[k], (*b).s[l]), 1, "main [check5.c]",
            "assign_sblk2s() changes the input field");
  ...
  for (n = 0; n < nb; n++) {
    for (iset = 0; iset < (int)(PT_SETS); iset++) {
      ...
      error(check_sdb(b, set, k, psd[0]), 1, "main [check5.c]",
            "assign_sdblk2sd() is incorrect");

      error(diff_sd(vol, (*b).sd[k], (*b).sd[l]), 1, "main [check5.c]",
            "assign_sdblk2sd() changes the input field");
