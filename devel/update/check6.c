...
static void divide_pf(double mu, int isp, int *status)
{
  ...
  if (sp.solver == CGNE) {
    ...
    error_root(status[0] < 0, 1, "divide_pf [check6.c]",
               "CGNE solver failed (parameter set no %d, status = %d)", isp,
               status[0]);
    ...
    error_root(status[0] < 0, 1, "divide_pf [check6.c]",
               "SAP_GCR solver failed (parameter set no %d, status = %d)", isp,
               status[0]);
    ...
    error_root((status[0] < 0) || (status[1] < 0), 1, "divide_pf [check6.c]",
...
int main(int argc, char *argv[])
{
  ...
  for (irw = 1; irw < 5; irw++) {
    for (isp = 0; isp < 3; isp++) {
      ...
      if (isp == 2) {
        dfl_modes(status);
        error_root(status[0] < 0, 1, "main [check6.c]", "dfl_modes failed");
      }
      ...
      if ((irw & 0x1) == 1)
        ...
        act0 = mu1 * mu1 * (mu2 * mu2 - mu1 * mu1) *
               action4(mu1, 0, 0, isp, 1, status);
        act0 += 2.0 * mu2 * mu2 * mu2 * mu2 *
                action4(sqrt(2.0) * mu2, 0, 0, isp, 1, status);
