...
static void alloc_X(void)
{
  ...
  error((X == NULL) || (sf == NULL), 1, "alloc_X [check5.c]",
  ...
  for (i = 0; i < 2; i++) {
    for (j = 0; j < 2; j++) {
      for (k = 0; k < 3; k++) {
        ddf[i][j][k].re +=
            0.25 *
            (xt[i] * xt[j] * (*sf).ptt[k].re + xt[i] * yt[j] * (*sf).ptd[k].re +
             yt[i] * xt[j] * (*sf).ptd[k].re + yt[i] * yt[j] * (*sf).pdd[k].re);

        ddf[i][j][k].im +=
            0.25 *
            (xt[i] * xt[j] * (*sf).ptt[k].im + xt[i] * yt[j] * (*sf).ptd[k].im +
             yt[i] * xt[j] * (*sf).ptd[k].im + yt[i] * yt[j] * (*sf).pdd[k].im);
      }
    }
  }
}
