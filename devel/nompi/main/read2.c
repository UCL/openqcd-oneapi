...
static void cnfg_range(FILE *fdat, int *fst, int *lst, int *stp)
{
  ...
  error(ie != 0, 1, "cnfg_range [read2.c]",
        "Non-contiguous configuration numbers");
}
...
static void select_cnfg_range(FILE *fdat)
{
  ...
  error((last < first) || (((last - first) % step) != 0) ||
            (((first - fst) % stp) != 0),
        1, "select_cnfg_range [read2.c]", "Improper configuration range");
...
static void print_plot(char *fin)
{
  ...
  error(fout == NULL, 1, "print_plot [read2.c]", "Unable to open output file");
