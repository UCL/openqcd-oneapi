...
{
    dpct::atomic_fetch_add(&(r.c1.re[idx]), s.c1.re);
    dpct::atomic_fetch_add(&(r.c1.im[idx]), s.c1.im);
    dpct::atomic_fetch_add(&(r.c2.re[idx]), s.c2.re);
    dpct::atomic_fetch_add(&(r.c2.im[idx]), s.c2.im);
    dpct::atomic_fetch_add(&(r.c3.re[idx]), s.c3.re);
    dpct::atomic_fetch_add(&(r.c3.im[idx]), s.c3.im);
}
...
{
    dpct::atomic_fetch_add(&(r.c1.re[idx]), -s.c1.im);
    dpct::atomic_fetch_add(&(r.c1.im[idx]), s.c1.re);
    dpct::atomic_fetch_add(&(r.c2.re[idx]), -s.c2.im);
    dpct::atomic_fetch_add(&(r.c2.im[idx]), s.c2.re);
    dpct::atomic_fetch_add(&(r.c3.re[idx]), -s.c3.im);
    dpct::atomic_fetch_add(&(r.c3.im[idx]), s.c3.re);
}
...
extern "C" 
void su3_AoS2SoA(int vol, su3_soa uout, su3 *uin, sycl::nd_item<3> item_ct1)
{
    int idx = item_ct1.get_group(2) * item_ct1.get_local_range().get(2) +
              item_ct1.get_local_id(2);
    if (idx >= vol/2) return;
...
void Dw_cuda_SoA(int VOLUME, su3 *u, spinor *s, spinor *r, pauli *m, int *piup, int *pidn)
{
    ...
    */
    start_ct1 = std::chrono::steady_clock::now(); // Start the timer
    q_ct1.memcpy(d_u_aos, u, 4 * VOLUME * sizeof(su3))
        .wait(); // Mem copy AoS H2D
    block_size = 128;
    ...
    */
    start_ct1 = std::chrono::steady_clock::now(); // Start the timer
    q_ct1.memcpy(d_s_aos, s, VOLUME * sizeof(spinor))
        .wait(); // Mem copy AoS H2D
    block_size = 128;
    grid_size = ceil(VOLUME/(float)block_size);
    /*
    ...
    */
    stop = q_ct1.parallel_for(
        sycl::nd_range<3>(sycl::range<3>(1, 1, grid_size) *
                          ...
                          sycl::range<3>(1, 1, block_size)),
        [=](sycl::nd_item<3> item_ct1) {
            deo_kernel(VOLUME, d_s_soa, d_r_soa, d_u_soa, d_piup, d_pidn, ceo,
                       one_over_gammaf, item_ct1);
        });
    /*
