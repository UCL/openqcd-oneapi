...
static void start_hmc(double *act0, su3_dble *uold, su3_alg_dble *mold)
{
  ...
  if (dfl.Ns) {
    ...
    error_root(status[0] < 0, 1, "start_hmc [hmc.c]",
               "Deflation subspace generation failed (status = %d)", status[0]);
  ...
  for (i = 0; i < nact; i++) {
    ...
    if (ap.action == ACG) {
      ...
      if (ap.action == ACF_TM1) {
        ...
        status[2] = 0;
        act0[n] =
            setpf5(mu[ap.imu[0]], mu[ap.imu[1]], ap.ipf, ap.isp[1], 0, status);
        chk_mode_regen(ap.isp[1], status);
        add2counter("field", ap.ipf, status);
...
int main(int argc, char *argv[])
{
  ...
  for (icnfg = first; icnfg <= last; icnfg += step) {
    ...
    for (i = 0; i < 4; i++) {
      set_hmc_parms(hmc.nact, hmc.iact, hmc.npf, hmc.nmu, hmc.mu, hmc.nlv,
                    tau[i]);
      set_mdsteps();
