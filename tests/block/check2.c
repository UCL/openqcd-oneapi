...
static int check_blk(block_t *b0, block_t *b, int iu, int iud, int ns, int nsd,
                     int shf)
{
  int vol;
...
static int check_bnd(block_t *b0, block_t *b, int iub, int iudb, int nw,
                     int nwd, int shf)
{
  int vol, ifc;
  bndry_t *bb0, *bb;
  ...
  for (igr = 0; igr < (int)(BLK_GRIDS); igr++) {
    ...
    for (n = 0; n < nb; n++) {
      ...
      error(((*b).bs[0] != bs[0]) || ((*b).bs[1] != bs[1]) ||
                ((*b).bs[2] != bs[2]) || ((*b).bs[3] != bs[3]),
            1, "main [check2.c]", "b.bs is incorrect");

      error(((*b).bo[0] < 0) || (((*b).bo[0] + bs[0]) > L0) ||
                ((*b).bo[1] < 0) || (((*b).bo[1] + bs[1]) > L1) ||
                ((*b).bo[2] < 0) || (((*b).bo[2] + bs[2]) > L2) ||
                ((*b).bo[3] < 0) || (((*b).bo[3] + bs[3]) > L3),
            1, "main [check2.c]", "b.bo is out of range");

      error((((*b).bo[0] % bs[0]) != 0) || (((*b).bo[1] % bs[1]) != 0) ||
                (((*b).bo[2] % bs[2]) != 0) || (((*b).bo[3] % bs[3]) != 0),
            1, "main [check2.c]", "b.bo is not an integer multiple of bs");
      ...
      error(((isw == 0) && (n >= (nb / 2))) || ((isw == 1) && (n < (nb / 2))),
