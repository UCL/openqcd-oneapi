...
int main(int argc, char *argv[])
{
  ...
  error(my_rank != ipr_global(cpr), 1, "main [check1.c]",
        "Processor coordinates are incorrect");
  ...
  error(itest == 2, 1, "main [check1.c]", "Processes are not properly blocked");
    ...
    for (x1 = 0; x1 < L1; x1++) {
      for (x2 = 0; x2 < L2; x2++) {
        for (x3 = 0; x3 < L3; x3++) {
          ...
          iy0 = iup[ix][0];
          iz0 =
              ipt[x3 + L3 * x2 + L2 * L3 * x1 + L1 * L2 * L3 * ((x0 + 1) % L0)];
          ...
          iy1 = iup[ix][1];
          iz1 =
              ipt[x3 + L3 * x2 + L2 * L3 * ((x1 + 1) % L1) + L1 * L2 * L3 * x0];
          ...
          iy2 = iup[ix][2];
          iz2 =
              ipt[x3 + L3 * ((x2 + 1) % L2) + L2 * L3 * x1 + L1 * L2 * L3 * x0];
          ...
          iy3 = iup[ix][3];
          iz3 =
              ipt[((x3 + 1) % L3) + L3 * x2 + L2 * L3 * x1 + L1 * L2 * L3 * x0];
          ...
          iy1 = idn[ix][1];
          iz1 = ipt[x3 + L3 * x2 + L2 * L3 * ((x1 + L1 - 1) % L1) +
                    L1 * L2 * L3 * x0];
          ...
          iy2 = idn[ix][2];
          iz2 = ipt[x3 + L3 * ((x2 + L2 - 1) % L2) + L2 * L3 * x1 +
                    L1 * L2 * L3 * x0];
          ...
          iy3 = idn[ix][3];
          iz3 = ipt[((x3 + L3 - 1) % L3) + L3 * x2 + L2 * L3 * x1 +
                    L1 * L2 * L3 * x0];
  ...
  error(itest == 1, 1, "main [check1.c]",
        "The index ipt does not respect eo ordering");
  error(itest == 2, 1, "main [check1.c]",
        "The index iup is out of range at the boundaries");
  error(itest == 3, 1, "main [check1.c]",
        "The index iup (combined with map) is incorrect");
  error(itest == 4, 1, "main [check1.c]",
        "The index idn is out of range at the boundaries");
  error(itest == 5, 1, "main [check1.c]",
        "The index idn (combined with map) is incorrect");
